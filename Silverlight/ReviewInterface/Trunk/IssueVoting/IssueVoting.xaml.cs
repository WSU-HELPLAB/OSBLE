    ...
    public partial class IssueVoting : UserControl
    {
        ...
        private void OpenPeerReviewDocuments()
        {
            ...
            if (documentsOpened == true && PeerReviewDocuments != null)
            {
                ...
                {
                    ReadPeerReview(XDocument.Load(peerReview.Stream), peerReview.Author, peerReview.Role, peerReview.Id);
                }
            }
        }

        private void ReadPeerReview(XDocument xDoc, string author, Classification role, int peerReviewID)
        {
            ...
            foreach (XElement document in ele)
            {
                ...
                foreach (IDocumentHolderViewModel DocumentHolder in mpVM.DocumentHolderViewModels)
                {
                    ...
                    {
                        DocumentHolder.LoadIssueVotingComments(document, noteAuthor, peerReviewID);
                    }
                }
            }
        }
